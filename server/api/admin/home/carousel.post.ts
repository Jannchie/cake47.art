...
export default defineEventHandler(async (event) => {
  ...
  const slotKey = `${CAROUSEL_PREFIX}${body.artworkId}`
  ...
  const dup = await db
    ...
    .get()
  if (dup) {
    throw createError({ statusCode: 409, statusMessage: 'Already in carousel' })
  }

  const max = await db
    .select({ value: sql<number>`coalesce(max(${tables.homeSlots.position}), -1)`.as('value') })
    .from(tables.homeSlots)
    .where(like(tables.homeSlots.slotKey, `${CAROUSEL_PREFIX}%`))
    .get()
